#line 2 "GreedyTravelingSalesman.cpp"
/*
 * Author : Yang Zhang
 */
#include <map>
...
class GreedyTravelingSalesman
{
    ...
    int worstDistance(vector <string> thousands, vector <string> hundreds, vector <string> tens, vector <string> ones) {
        ...
        for (int i = 0; i < n; i++)
            for (int j = 0; j < n; j++) {
                g[i][j] = thousands[i][j] - '0';
                g[i][j] = g[i][j] * 10 + hundreds[i][j] - '0';
                g[i][j] = g[i][j] * 10 + tens[i][j] - '0';
                g[i][j] = g[i][j] * 10 + ones[i][j] - '0';
            }
        int ret = -1;
        for (int i = 0; i < n; i++)
    ...
    int dfs(int cur, int u, int v) {
        ...
        if (!edge && (cur == u || cur == v)) {
            ...
            if (Min == inf) temp = 9999;
            else if (nxt < x) temp = Min;
            else temp = Min - 1;
            if (temp < 1) return -1;
            return temp + dfs(nxt, u, v);
        }
        Min = inf, x = n;
        for (int i = 0; i < n; i++)
        if (!vst[i]) {
