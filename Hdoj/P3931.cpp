...
int main() {
	...
	while (scanf("%d%d%d%d", &L, &W, &N, &P) != EOF) {
		...
		sap.init(2 * N + 2);
		for (int i = 1; i <= N; i++) {
			sap.addCap(i, i + N, p[i]);
			if (y[i] - r[i] <= 0) sap.addCap(source, i, inf);
			if (y[i] + r[i] >= W) sap.addCap(i + N, sink, inf);
		}
		for (int i = 1; i <= N; i++)
			for (int j = i + 1; j <= N; j++) {
				if (inter(i, j)) {
		...
		if (ret >= P) printf("Our hero has been killed\n");
		else printf("%d\n", P - ret);
	}
	return 0;
}
