...
int n;
vector<hiker> vec;
int encounter(const vector<hiker> &vhiker) {
	if (vhiker[0].m < vhiker[1].m) {
		LL s1 = (vhiker[0].d == vhiker[1].d) ? 720 : vhiker[1].d - vhiker[0].d + 360, s2 = 360 - vhiker[1].d;
		if (s1 * vhiker[0].m <= s2 * (vhiker[1].m - vhiker[0].m)) {
			return 1;
		} else {
			return 0;
		}
	} else { // hiker2 is faster
		LL s1 = 360 - vhiker[1].d + vhiker[0].d, s2 = 360 - vhiker[0].d;
		if (s1 * vhiker[1].m <= s2 * (vhiker[0].m - vhiker[1].m)) {
			return 1;
		} else {
			return 0;
		}
