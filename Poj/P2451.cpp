...
int main() {
	...
	while (~scanf("%d", &n)) {
		int cnt = 0;	
		add_hplane(Point(0, 0), Point(MAXP, 0), cnt);
		add_hplane(Point(MAXP, 0), Point(MAXP, MAXP), cnt);
		add_hplane(Point(MAXP, MAXP), Point(0, MAXP), cnt);
		add_hplane(Point(0, MAXP), Point(0, 0), cnt);
		for (int i = 0; i < n; i++) {
			Point a, b;
			a.input(); b.input();
			add_hplane(a, b, cnt);
