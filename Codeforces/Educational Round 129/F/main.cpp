...
void solve() {
    ...
    cin >> n;
    LinkCut lct(n);
    for (int i = 1; i <= n; i++) lct.Update(i, 1);
    vector<vector<pair<int, int>>> eds(n);
    for (int i = 1; i < n; i++) {
        int u, v, w;
        cin >> u >> v >> w;
        w--;
    ...
    LL res = 0;
    for (int i = 0; i < n; i++) {
        for (auto& e: eds[i]) lct.Cut(e.first, e.second);
        for (auto& e: eds[i]) {
            lct.reroot(e.first);
            LL l = lct.Subtree(e.first, e.second);
            lct.reroot(e.second);
            LL r = lct.Subtree(e.second, e.first);
            res += l * r;
        }
        for (auto& e: eds[i]) lct.Link(e.first, e.second);
    }
    cout << res << endl;
}
